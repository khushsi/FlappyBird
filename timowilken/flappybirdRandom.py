...
def main():
    ...
    pygame.display.set_caption('Pygame Flappy Bird - kmt81')
...
#                     bird.msec_to_climb = Bird.CLIMB_DURATION
            ...
            ## Current State
            currX = grid(pipes[-1].x - bird.x) 
            currY = grid(( WIN_HEIGHT - (pipes[-1].bottom_pieces + 1 ) * 32 ) - bird.y)            
            currState = cgame.getState(currX, currY)
            ...
            pygame.display.flip()
            frame_clock += 1
        pickle.dump(cvisitMap, open( "visitedMapR.pickle", "wb" ) )
        pickle.dump(cstatMap, open( "stateMapR.pickle", "wb" ) )
        pickle.dump( crewardMap, open( "rewardMapR.pickle", "wb" ) )    
